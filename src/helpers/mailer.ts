        ...
        const transport = nodemailer.createTransport({
          ...
          // });
            ...
            const mailOptions = {
                ...
                html: `
                <h1>${emailType === "VERIFY" ? "Verify Your Email" : "Reset password"}</h1>
                <p>Click on the link below to ${emailType === "VERIFY" ? "verify your email" : "reset your password"}</p>
                <a href="${process.env.DOMAIN}/${emailType === "VERIFY" ? "verifyemail" : "resetpassword"}?token=${token}">${emailType === "VERIFY" ? "Verify Email" : "Reset Password"}</a>
